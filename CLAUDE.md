...
## #11
**Паритет tg/web.** У проекта две поверхности — Telegram-бот (`src/`) и
веб-приложение (`web/`). Функциональность, добавленная в одну версию,
обязана появиться и в другой в том же PR/сессии, если только она не помечена
в соответствующей `/spec/<module>.md` как `[WEB-ONLY: причина]` или
`[TG-ONLY: причина]` (например, «жест открытия камеры» — web-only: у бота
нет вызова камеры устройства помимо аплоада фото в чат). Пропустить вторую
версию молча нельзя: либо реализовать, либо явно задокументировать причину
и завести задачу в `TODO.md`.
