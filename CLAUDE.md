...
В проекте есть граф знаний в `graphify-out/`: узлы-«хабы», структура сообществ,
межфайловые связи.

Правила:
- Для вопросов по кодовой базе сначала выполнить `graphify query "<вопрос>"`,
  если существует `graphify-out/graph.json`. Для связей — `graphify path "<A>" "<B>"`,
  для конкретного понятия — `graphify explain "<концепт>"`. Это даёт узкий
  подграф, обычно намного компактнее, чем `GRAPH_REPORT.md` или сырой grep.
- Если есть `graphify-out/wiki/index.md` — использовать его для общей навигации
  вместо просмотра исходников.
- `graphify-out/GRAPH_REPORT.md` читать только для общего обзора архитектуры
  или когда query/path/explain не дали достаточно контекста.
- После изменения кода выполнить `graphify update .`, чтобы граф оставался
  актуальным (только AST, без обращения к LLM).
