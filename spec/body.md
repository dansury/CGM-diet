...
`[TG-ONLY: в веб-приложении дневного коридора нет вовсе — вес там только
записывается и рисуется графиком, цели и ориентира по калориям нет. Перенос —
это весь модуль (BMR, TDEE, границы диетологов, полосы), а не одна строка;
задача — T087.]`
...
Одна другую не заменяет и не отменяет.
...
## Цель без известного веса

Темп считается от текущего веса, поэтому цель, названная раньше первого
взвешивания, не строится сразу — но и не теряется: `offer_goal` кладёт её в
FSM (`PENDING_GOAL_KEY`), переводит диалог в вопрос о весе, а `on_value` после
удачной записи веса сам возвращается к `offer_goal`. `save_weight_entry`
возвращает `bool`: вес вне 25–400 кг не записан, к цели не возвращаемся.
