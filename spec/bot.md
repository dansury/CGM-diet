...
полоса дневного коридора (`spec/body.md`), калорийная полоса этого приёма и
оценка пропорций тарелки (обе — `spec/plate.md`); все три части не обязательны
и не могут отменить запись. Под сообщением —
