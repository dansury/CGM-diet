# dictionary — «мои блюда», подсказки по первым буквам, память БЖУ

Цель: то, что человек ест и принимает регулярно, вводится **одним нажатием**,
а не фотографией и подтверждением каждый раз.

Название в интерфейсе — **«мои блюда»** (и у бота, и в web). В коде и в
схеме сущность по-прежнему `dictionary` / `user_dictionary`: переименование
касается только того, что видит человек.

## Что попадает в «мои блюда»

Все именованные сущности, которые человек может повторить, — один список.
...
Сахар, вес и анализы в «мои блюда» не попадают: это числа, а не имена — повторять
кнопкой нечего.
