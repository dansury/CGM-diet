...
- `kcal` пустой, а БЖУ есть → Атуотер `4·Б + 9·Ж + 4·У`; сумма Атуотера
  нулевая (модель честно поставила нули, как углеводы в мясе), но блюдо есть
  в таблице → `kcal = Ref.kcal × порция/100`: у существующего блюда калорий
  не бывает нулевых;
