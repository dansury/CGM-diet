...
Тот же переключатель управляет напоминанием через 60–75 мин после еды
(T040, `src/scheduler.py::run_sugar_reminders`, тик — `NOTIFY_TICK_SECONDS`):
исходная подсказка обещает «через 1–2 часа», но к этому моменту обычно
теряется в переписке, поэтому бот напоминает отдельным сообщением
(`reporting.format_sugar_reminder()`, кнопка `sg:log`), если замера для этой
еды ещё нет. Отметка о напоминании — `meals.sugar_reminder_sent_at`,
ставится до отправки, чтобы повторный тик не задвоил. Тихие часы
(`spec/notifications.md`) действуют как у остальных системных напоминаний.

`[TG-ONLY: у web-уведомлений (`web/lib/notifications.php`) есть только
расписание по времени суток (`spec/notifications.md`), понятия «через N минут
после события X» там нет — это отдельная инфраструктура того же размера, что
и сама задача T040. Задача на web-паритет — T082.]`
