...
web/
  ...
  tests/llm_chain.php       `php web/tests/llm_chain.php` — кого цепочка зовёт,
...
2. `recognize.php`: `LLM::visionJson(system, userText, [dataUrl], …)` (фото)
   ...
   (§ Модель / LLM → Цепочка кандидатов). Сбой пишется в лог целиком: вход,
   каждый кандидат с ответом провайдера и снимок конфигурации (§ Лог).
   Пользователю уходит одна строка `LLM::failureReason()` — причина на каждого
   провайдера, не нумерованная цепочка:
   «Не удалось распознать (yandex: модель не включена в каталоге облака;
   openrouter: запрос блокирует хостинг). Попробуйте ещё раз или опишите блюдо
   текстом.»
...
Причина, по которой каталог не пришёл, переводится на человеческий
(`model_catalog_hint`): `Access denied by security policy` — это защита
хостинга, закрывшая исходящие запросы, а не отказ провайдера и не плохой ключ.
...
`LLM::candidateChain(vision)->{primary, candidates[], skipped[]}` — чистая
функция над конфигом, без сети; `LLM::dispatch()` идёт ровно по ней. Порядок:
выбранная модель → (`LLM_FALLBACK_MODE=auto`) её более новые версии →
`LLM_FALLBACK_MODELS` → на фото весь пул моделей со зрением → «последняя
попытка» каждого провайдера в порядке `LLM_PROVIDER_PRIORITY`.

Из цепочки выбывают, каждый с причиной в `skipped`:

```
провайдер не назвал эту модель в своём каталоге   # live=false у провайдера,
                                                  # ответившего на GET /models
модель не принимает изображения                   # только на шаге vision
```

Вшитый `AVAILABLE_MODELS` за слаг не ручается — он стареет вместе с релизом,
живой ответ нет. Провайдер, каталог которого не пришёл, ничего не доказывает:
его строки остаются в цепочке. **Выбранная** модель не выбывает никогда —
это явное решение владельца, и админка уже пометила её `⛔`.

«Последняя попытка» (`LLM_FALLBACK_MODEL`, `YANDEX_FALLBACK_MODEL`) ищется в
каталоге и идёт в цепочку найденной строкой — со своими `live` и `vision`:
слепой слаг без модальности отправлял фото в текстовую модель, а та отвечала
`empty message text`, и это читалось как наша ошибка.

Во время обхода нога провайдера закрывается целиком, если отказ не про модель
(`LLMHttpError::providerWide()`): HTTP 401/407 — ключ, HTTP 403 не в конверте
провайдера (`{"error":{…}}`) — запрет на подступах, хостинг или прокси.
Остальные кандидаты этого провайдера пропускаются с той же причиной. HTTP 403
в конверте провайдера и `Failed to get model` — про одну модель, обход
продолжается.

Проверяется офлайн: `php web/tests/llm_chain.php`.

`[WEB-ONLY: у бота нет ни второго провайдера, ни живого каталога — `src/llm/`
ходит только в OpenRouter. Его цепочка (`src/llm/fallback.py`,
`spec/models.md`) переключается на 429 и таймаут, а прочие `LLMError`
пробрасывает сразу, поэтому мёртвая модель не съедает попытки — фильтровать
там нечего. Появится каталог провайдера в боте — фильтр придётся завести
и там.]`
