...
🏷 <b>Две стороны упаковки</b> — отправьте оба фото одним альбомом, я соберу
их в одну карточку продукта.
...
⭐️ <b>Мои блюда</b> (/my) — всё, что вы записываете повторно: блюда,
продукты, упаковки, лекарства и самочувствие. Сверху всегда последнее.

🔔 <b>Напоминания</b> (/notify) — время и вид каждого напоминания. «Умное»
подбирает время по вашим же записям: когда вы обычно едите или меряете сахар.

❌ <b>Отменить</b> — крестик есть на любой карточке и в любом приглашении
что-то написать; то же делает /cancel.

🎯 <b>Мои цели</b> — зачем вы здесь: снизить вес, держать сахар, наладить питание,
разобраться с самочувствием или свой вариант. От этого зависит, о чём я говорю
в первую очередь; поменять — кнопкой «🎯 Мои цели» в /body.
...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    if is_first_run:
        ...
        return
    # Одна возможность про запас — только тому, кто уже знаком с ботом
    # (`spec/features.md`). Новичку в первую минуту рассказывать о том, чем он
    # ещё не пользовался, нечего: у него на экране анкета, а не свободные руки.
    if message.bot is not None:
        try:
            await maybe_send_hint(message.bot, message.chat.id)
        except Exception:
...
async def cmd_settings(message: Message) -> None:
    async with session_scope() as session:
        ...
        text = (
            ...
            f"Замер после еды: {'предлагать и напомнить через час' if user.glucose_prompt_enabled else 'не предлагать'}\n"
            f"Недельный дайджест: {'по понедельникам' if user.weekly_digest_enabled else 'выкл'}\n"
            f"Приёмов пищи в день: {meals}\n"
            "Напоминания (время и вид каждого): /notify\n\n"
            ...
            "<code>/set sugar on|off</code>, <code>/set week on|off</code>"
        )
...
def _apply_setting(user, key: str, value: str) -> str:
    ...
    if key == "sleep":
        ...
        raise ValueError("нужно on или off")
    if key == "sugar":
        # Предложение прислать замер после каждой записи еды и напоминание
        # через 60–75 мин, если замера ещё нет (T040), — один переключатель
        # (`spec/onboarding.md` § Сахарный трек).
        if value.lower() in {"on", "вкл", "да", "1", "true"}:
            user.glucose_prompt_enabled = True
            return "замер после еды: буду предлагать и напомню через час"
        if value.lower() in {"off", "выкл", "нет", "0", "false"}:
            user.glucose_prompt_enabled = False
            return "замер после еды: больше не предлагаю и не напоминаю"
        raise ValueError("нужно on или off")
    if key == "week":
        # Недельный дайджест (`spec/analytics.md` § Недельный дайджест): одно
        # письмо в неделю, единственное место, где бот говорит «что изменилось».
        if value.lower() in {"on", "вкл", "да", "1", "true"}:
            user.weekly_digest_enabled = True
            return "недельный дайджест: по понедельникам утром"
        if value.lower() in {"off", "выкл", "нет", "0", "false"}:
            user.weekly_digest_enabled = False
            return "недельный дайджест: выключен — /week работает по-прежнему"
        raise ValueError("нужно on или off")
    if key == "meals":
