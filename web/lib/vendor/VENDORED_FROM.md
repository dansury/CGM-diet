...
None. Everything in this directory is byte-identical to
`site_yacloud_openrouter`; a re-copy from upstream loses nothing.
...
Sync commit: 8b6b088 (`site_yacloud_openrouter`, branch
`claude/todo-md-cgm-uzvjfp`) — the candidate chain that does not spend attempts
on models the provider's own live catalogue does not list (`LLMHttpError` with
`providerWide()`, `LLM::candidateChain()`, a provider skipped wholesale after a
non-model refusal, `LLM::failureReason()` for the end user) and the
`qwen3.6-35b-a3b` catalogue row that is also the default `LLM_VISION_MODEL`.
Written here first because the upstream repository was not reachable from that
session; carried upstream with its spec update and `tests/llm_chain.php`, then
re-copied back — this mirror and upstream are equal again.

Previous sync commit: 32614c3 (`site_yacloud_openrouter`, branch
